...
export class TidPage implements OnInit {
  MeterNum:any;
  SGCF:any;
  KRNF:any;
  TIF:any;
...
ionViewWillLeave(){
 ...
 // checkReff()
  ...
  InsertRateCard()
  {
    try
    {
      this.ClearCredit = this.httpClient.get("http://services.webvend.co.za/MobileAppService/api/KeyChange?NoTokens=1&MeterNo="+this.MeterNum+"&SGC="+this.SGCF+"&KRN=1&TI=1&ToSGC="+this.SGCF+"&ToTI=1&ToKRN=2"+"&UserName="+this.UserName+"&Password="+this.Password+"&Resource="+this.Resource+"");
      this.ClearCredit.subscribe((data) => 
      {
        this.LocalClearCredit = Object.values(data);
        var stringifiedData = JSON.stringify(data);
        ...
        var t2 = tokens[1].toString().replace('Token 2 :','').replace('"','');
        this.Token2 = t2.toString().replace('"','');
        

        //console.log("got response");
        this.gotResponse=false;
        this.Message = "Please enter tokens "+this.Token+" into Meter: "+this.MeterNum+" to perform TID change"
