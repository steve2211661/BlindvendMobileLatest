...
@Component({
  selector: 'app-load-limit',
  templateUrl: './load-limit.page.html',
  styleUrls: ['./load-limit.page.scss'],
})
export class LoadLimitPage implements OnInit {
  MeterNum:any;
  SGC:any;
  KRN:any;
  TI:any;
  ...
  InsertRateCard()
  {
    try
    {
      this.ClearCredit = this.httpClient.get("http://services.webvend.co.za/MobileAppService/api/LoadLimit?NoTokens=1&MeterNo="+this.MeterNum+"&SGC="+this.SGC+"&KRN="+this.KRN+"&TI="+this.TI+"&LoadLimit="+this.LoadLimit+"&UserName="+this.UserName+"&Password="+this.Password+"");
      this.ClearCredit.subscribe((data) => 
      {
        this.LocalClearCredit = Object.values(data);
        var stringifiedData = JSON.stringify(data);
        ...
        this.Message = "Please enter token: "+this.Token+" into Meter: "+this.MeterNum+" to change load limit."
